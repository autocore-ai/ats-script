...
def test_planning_testcase(planning_env, name="test_01", gt_name="gt_01"):
    ...
    /tmp/.X11-unix:/tmp/.X11-unix:rw -v $HOME/.Xauthority:$HOME/.Xauthority:rw -e ROS_MASTER_URI=$
    {ROS_MASTER_URI} -e ROS_IP=${ROS_IP} -e DISPLAY=${DISPLAY} -e XAUTHORITY=${XAUTH}
    autocore/simulator_for_sdk
    ...
    with allure.step("collect ground_truth bag data"):
        ...
        #     print(topic)
        #     keyw = topic.split("/")
        #     assert topic_csv(gt_name+".bag", topic, "gt_"+keyw[-1],
        #     conf.LOCAL_GT_BAG_PATH), topic+" could not saved to csv file"
        #     time.sleep(2)
        #
    ...
    with allure.step(step_5):
        ...
        for topic in TOPICS.split(" "):
            print(topic)
            keyw = topic.split("/")
            topic_bag = bag_name + ".bag"
            localb_address = conf.LOCAL_TEST_BAG_PATH
            assert topic_csv(topic_bag, topic, keyw[-1], localb_address), topic + " could not saved to csv file"
            time.sleep(2)

    bag_v_path = conf.LOCAL_TEST_BAG_PATH + "test_01_twist.csv"
    gt_v_path = conf.LOCAL_GT_BAG_PATH + "gt_01_twist.csv"

    bag_pose_path = conf.LOCAL_TEST_BAG_PATH + "test_01_current_pose.csv"
    gt_pose_path = conf.LOCAL_GT_BAG_PATH + "gt_01_current_pose.csv"
    ...
    gt_trajectory = conf.LOCAL_GT_BAG_PATH + "gt_01_trajectory.csv"
    t_trajecotry = conf.LOCAL_TEST_BAG_PATH + "test_01_trajectory.csv"

    gt_route = conf.LOCAL_GT_BAG_PATH + "gt_01_route.csv"
    ...
    with allure.step("Data analysis"):
        with allure.step("1. 如果输出文档速度一直为 0   -> not pass"):
            a_file = pd.read_csv(bag_v_path)
            assert velocity_not_zero(a_file)

        with allure.step("2. current_pose 值一直没有变 -> not pass"):
            b_file = pd.read_csv(bag_pose_path)
            assert current_pose_change(b_file)

        with allure.step("3.current_pose: 两值比较： 1.欧式距离之差 -》 超过一个range， 不pass "):
            df1, df2 = csv_to_df(gt_pose_path, bag_pose_path)
            logger.info(df1.shape[0])
            logger.info(df2.shape[0])
            count_shape = df1.shape[0] - df2.shape[0]
            df1.drop(df1.tail(count_shape).index, inplace=True)
            logger.info(df1.shape[0])
        ...
        with allure.step("6.trajectory 偏航角之差 做图，前40的points"):
            trajectory_yaw_plot(gt_trajectory, t_trajecotry)
        ...
        with allure.step("8. /planning/mission_planning/route   信息匹配"):
            assert route_same(gt_route, test_route), "planning_route msg is not the same "
...
if __name__ == '__main__':
    ...
    # 生成报告
    print('=' * 20)
    # generate = 'allure generate /home/minwei/autotest/allure_reports/
    # result/ -o /home/minwei/autotest/allure_reports/report/ --clean'
